...
class MyGatesDevicesOutputs extends PureComponent {
    ...
    componentDidMount (){
        ...
        http.get(`/api/method/iot_ui.iot_api.gate_device_cfg?sn=${this.props.match.params.sn}&vsn=${this.props.match.params.vsn}&_=${new Date() * 1}`).then(res=>{
            console.log(res);
            this.setState({data: res.message.outputs})
        })
    }
    showModal = (record) => {
    ...
    render () {
        ...
        return (
            <div>
                ...
                >
                    <p className="flex">点名： <Input disabled defaultValue={this.state.record.name}/></p>
                    <p className="flex">数值： <Input onChange={(value)=>{
