...
class MyGatesAppsInstall extends Component {
    ...
    getConfig = (val)=>{
        ...
        deviceColumns && deviceColumns.length > 0 && deviceColumns.map((v, key)=>{
            ...
            data.push({
                ...
                render: (record) => (
                    <Button onClick={() => {
                        this.props.store.codeStore.delectDataSource(record)
                    }}>删除</Button>
                )
            });
    ...
    callback = (key)=>{
        key;
        if (this.state.config && this.state.config.length > 0 && this.state.config.hasOwnProperty('section')) {
            console.log('不可编辑');
            this.getData();
        } else {
            console.log('可编辑')
    ...
    render () {
        const { vendor, agreement, type, data, flag, item, detail, showTempLists, serial, tcp, dataSourceCode,
            addTempLists, instName, showTempList, config, addTempList} = this.state;
        console.log(showTempList)
        return (<div>
                <Status />
                            ...
                            >
                                                                        ...
                                                                        if (a.type === 'dropdown') {
                                                                            return (
                                                                                ...
                                                                                <span
                                                                                    ...
                                                                                    <Select
                                                                                        defaultValue={a.value ? a.value[0] : null}
                                                                                        style={{width: 300}}
                                                                                        onChange={() => {
                                ...
                                >
                                    ...
                                    <AceEditor
                                        ...
                                        value={JSON.stringify(dataSourceCode) !== '[]' && config.hasOwnProperty('section') ? JSON.stringify(dataSourceCode) : item.pre_configuration}
                                        fontSize={16}
